...
export class GoogleStorageService {
  ...
  public uploadFile(file: File): Observable<any> {
    var d = new Date();
    return this.http.post<any>( 'https://storage.googleapis.com/upload/storage/v1/b/' + this.config.bucket + '/o?uploadType=media&name=' + d.getMilliseconds() + '_' + d.getFullYear() + d.getMonth() + d.getDay() + d.getHours() + '/' + file.name, file);
  }

  public uploadDirect(file: File): Observable<MiaResponse<MiaFile>> {
    var d = new Date();
    return this.http.post<any>( 'https://storage.googleapis.com/upload/storage/v1/b/' + this.config.bucket + '/o?uploadType=media&name=' + d.getMilliseconds() + '_' + d.getFullYear() + d.getMonth() + d.getDay() + d.getHours() + '/' + file.name, file).pipe(map(data => {
      return {
        success: true,
        response: {
          name: file.name,
